...
# URL fixa - sem mostrar para o usuário
...
# Função para converter lat/lng no formato brasileiro (SÓ para o mapa)
...
# Gera mapa
if len(df_filt) > 0:
    # Cria uma cópia só para o mapa e converte as coordenadas
    df_mapa = df_filt.copy()
    df_mapa.loc[:, "lat"] = clean_lat_lng(df_mapa["lat"])
    df_mapa.loc[:, "lng"] = clean_lat_lng(df_mapa["lng"])
    
    # Remove apenas os registros sem coordenadas válidas para o mapa
