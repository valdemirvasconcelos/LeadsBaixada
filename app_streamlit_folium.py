...
def load_data(uploaded_file):
    """Carrega dados de CSV ou JSON com tratamento de erro."""
    try:
...
def generate_color_map_folium(categories):
    ...
    color_map = {}
    # Usar uma paleta de cores pré-definida ou gerar cores distintas
    # Gerando cores com base em hash para consistência
    for category in categories:
        hash_object = hashlib.md5(category.encode())
        hex_dig = hash_object.hexdigest()
        # Gera cor HEX a partir do hash
...
uploaded = st.sidebar.file_uploader(
    "Carregue seu arquivo de leads (CSV ou JSON)", type=["csv", "json"]
)
...
if uploaded:
    ...
    if df is not None:
        ...
        # Folium usa nomes de tiles, não URLs do Mapbox diretamente como Pydeck
        map_tiles_options = [
            "OpenStreetMap", "CartoDB positron", "CartoDB dark_matter",
            "Stamen Terrain", "Stamen Toner", "Stamen Watercolor"
        ]
        selected_tile = st.sidebar.selectbox("Estilo do Mapa (Tile)", map_tiles_options, index=1) # Default para positron
        ...
        if not df_filt.empty:
            ...
            try:
                ...
                # Ajusta zoom baseado na dispersão (simples)
                ...
                map_center = [-23.9, -46.4] # Fallback para Baixada Santista
                zoom_start = 9

            # Cria o mapa Folium
            m = folium.Map(location=map_center, zoom_start=zoom_start, tiles=selected_tile)

            # Gera mapa de cores para as categorias selecionadas
            color_map = generate_color_map_folium(cat_sel)

            # Adiciona marcadores ao mapa
            for idx, row in df_filt.iterrows():
                # Prepara dados para tooltip (trata NaNs)
                ...
                # Adiciona o marcador circular
                folium.CircleMarker(
                    location=[row["lat"], row["lng"]],
                    radius=radius_size, # Raio em pixels
                    popup=popup,
                    tooltip=nome, # Tooltip simples com o nome ao passar o mouse
                    color=color_map.get(row["categoria"], "#808080"), # Cor da borda (cinza padrão)
                    fill=True,
                    fill_color=color_map.get(row["categoria"], "#808080"), # Cor de preenchimento
                    fill_opacity=0.7
                ).add_to(m)
    ...
    st.info("👈 Por favor, carregue um arquivo CSV ou JSON na barra lateral para começar.")
